...
def plt_proc(data, color, alpha, label, lw, rate):
    ...
    # plt.plot(list(range(data[0]["acc"].shape[0]))[::rate], data[0]["acc"][::rate], color=color, alpha=alpha, label=label, linewidth=lw)
    # plt.plot(list(range(data[1]["acc"].shape[0]))[::rate], data[1]["acc"][::rate], color=color, alpha=alpha, label=None, linewidth=lw)
    # plt.plot(list(range(data[2]["acc"].shape[0]))[::rate], data[2]["acc"][::rate], color=color, alpha=alpha, label=None, linewidth=lw)
    plt.fill_between(
...
for mi in range(len(paths)):
    ...
    plt_proc(curves[mi][3], COLORS["rl_acc"], ALPHA, "$RL_A$", LW, RATE)
    plt_proc(curves[mi][1], COLORS["rl_raw"], ALPHA, "$RL_R$", LW, RATE)
    ...
    plt.tight_layout()
    filename = "rewards"
    plt.savefig("%s/rl_class_%s_%s.png"%(root_dir, filename, name_list[mi]), bbox_inches='tight', pad_inches=0.1)
    plt.close()
