...
class RoverEnv(base_env.BaseEnv):
    ...
    def init_x_cycle(self, N):
        ...
        # rover_x = torch.clamp(rover_x, 0, 10)
        # rover_y = torch.clamp(rover_y, 0, 10)
    ...
    def generate_stl(self):
        ...
        if args.hold_t > 0:
            ...
            # emergency = Imply(AP(lambda x: battery_limit - x[..., 6]), Eventually(0, args.nt, Always(0, args.hold_t, at_charger)))
    ...
    def visualize(self, x_init, seg, acc, epi):
        ...
        for i in range(row//2):
            for j in range(col):
                ...
                # ax = plt.subplot(row, col, (i * 2 + 1) * col + j + 1)
                # ax = ax_list[i(i * 2 + 1) * col + j]
                ax = ax_list[i*2+1, j]
    ...
    def generate_reward_batch(self, state):
        ...
        is_collide0 = check_in_box(state[:, 0:2], self.objs_np[1])
        is_collide1 = check_in_box(state[:, 0:2], self.objs_np[2])
        is_collide2 = check_in_box(state[:, 0:2], self.objs_np[3])
        is_collide3 = check_in_box(state[:, 0:2], self.objs_np[4])

        is_collide = np.logical_or(
            np.logical_or(is_collide0, is_collide1),
            np.logical_or(is_collide2, is_collide3)
        )
        out_of_map = np.logical_not(check_in_box(state[:, 0:2], self.objs_np[0]))
